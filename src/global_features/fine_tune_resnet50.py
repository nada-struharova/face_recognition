...
# 1. Set Dataset and Model
dataset_type = 'celeb_a'

# 2. Load Dataset
(ds, train_ds, val_ds, test_ds), num_classes = data_utils.load_dataset(dataset_type)
...
train_ds = train_ds.map(data_utils.preprocess_image_resnet50, num_parallel_calls=tf.data.AUTOTUNE)
val_ds = val_ds.map(data_utils.preprocess_image_resnet50, num_parallel_calls=tf.data.AUTOTUNE)
test_ds = test_ds.map(data_utils.preprocess_image_resnet50, num_parallel_calls=tf.data.AUTOTUNE)
